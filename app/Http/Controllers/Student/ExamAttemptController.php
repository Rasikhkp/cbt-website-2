...
class ExamAttemptController extends Controller
{
    ...
    // Method to pause attempt (for future use in Phase 6)
    public function pause(ExamAttempt $attempt)
    {
        $this->authorizeAttempt($attempt);

        if (!$attempt->isInProgress()) {
            return response()->json(['error' => 'Exam is not in progress'], 422);
        }
        ...
        $attempt->update([
            ...
            'session_data' => array_merge($attempt->session_data ?? [], [
    ...
    // Method to resume attempt (for future use in Phase 6)
    public function resume(ExamAttempt $attempt)
    {
        ...
        if ($attempt->time_remaining_seconds === null) {
            return response()->json(['error' => 'Exam was not paused'], 422);
        }

        // Resume with remaining time
        $newExpiryTime = now()->addSeconds($attempt->time_remaining_seconds);
